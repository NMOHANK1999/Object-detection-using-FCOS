...
def fcos_get_deltas_from_locations(
    ...
    """
    ##########################################################################
    # TODO: Implement the logic to get deltas from feature locations.        # DONE
    ##########################################################################
    ...
    ####
    deltas = deltas.to('cuda:0')
    gt_boxes = gt_boxes.to('cuda:0')
    locations = locations.to('cuda:0')
    ####
...
def fcos_apply_deltas_to_locations(
    ...
    """
    ##########################################################################
    # TODO: Implement the transformation logic to get boxes.                 #FIX THIS 
    #                                                                        #
    ...
    ##########################################################################
    output_boxes = torch.zeros([locations.shape[0], 4]).to('cuda:0')
    xc, yc = locations[:, 0].to('cuda:0'), locations[:, 1].to('cuda:0')
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    """
    ##########################################################################
    # TODO: Implement the centerness calculation logic.                      #FIX THIS 
    ...
    l, t, r, b = torch.abs(deltas[:, 0]), torch.abs(deltas[:, 1]), torch.abs(deltas[:, 2]),torch.abs(deltas[:, 3])
    centerness = torch.sqrt((torch.min(l, r) * torch.min(t, b))/(torch.max(l, r) * torch.max(t, b)))

    neg = -1 * torch.ones(4)
    neg = neg.to('cuda:0')
    for i in range(len(deltas)):
        if torch.all(deltas[i] == neg):  
            centerness[i] = -1
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ##################################################################–####
        # TODO: Implement logic to get location co-ordinates below.          #DONE
        ######################################################################
        B, C, W, H = feat_shape

        xc_yc = torch.stack(torch.meshgrid(torch.arange(H), torch.arange(W)), dim=-1).view((-1,2)) #not sure if the reshape is happening correctly
        xc_yc = xc_yc * level_stride + level_stride//2 

        location_coords[level_name] = xc_yc
