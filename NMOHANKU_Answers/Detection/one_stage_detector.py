...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params["lateral_c3"] = nn.Conv2d(dummy_out_shapes[0][1][1], out_channels, 1, 1, 0)
        self.fpn_params["lateral_c4"] = nn.Conv2d(dummy_out_shapes[1][1][1], out_channels, 1, 1, 0)
        self.fpn_params["lateral_c5"] = nn.Conv2d(dummy_out_shapes[2][1][1], out_channels, 1, 1, 0)
        
        self.fpn_params["output_p3"] = nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1)
        self.fpn_params["output_p4"] = nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1)
        self.fpn_params["output_p5"] = nn.Conv2d(self.out_channels, self.out_channels, 3, 1, 1)
    ...
    def forward(self, images: torch.Tensor):
        ...
        p4 = F.interpolate(p5, scale_factor = 2, mode="nearest") + self.fpn_params["lateral_c4"](backbone_feats["c4"])
        
        p3 = F.interpolate(p4, scale_factor = 2, mode="nearest") + self.fpn_params["lateral_c3"](backbone_feats["c3"])
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(self, num_classes: int, in_channels: int, stem_channels: List[int]):
        ...
        # pass
        ins = in_channels
        for chan in stem_channels:
            stem_cls.append(nn.Conv2d(ins, chan, 3, 1, 1))
            stem_cls.append(nn.ReLU())
            stem_box.append(nn.Conv2d(ins, chan, 3, 1, 1))
        ...
        self.pred_cls = nn.Conv2d(ins, num_classes, 3, padding=1)  # Class prediction conv
        self.pred_box = nn.Conv2d(ins, 4, 3, padding=1)  # Box regression conv
        self.pred_ctr = nn.Conv2d(ins, 1, 3, padding=1)  # Centerness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level in feats_per_fpn_level.keys():
            ...
            cls_logits = self.pred_cls(self.stem_cls(feats_per_fpn_level[level]))
            class_logits[level] = cls_logits.flatten(2).permute(0, 2, 1) #used to flatten along batch dimension and retain 3d structure

            box_deltas = self.pred_box(self.stem_box(feats_per_fpn_level[level]))
            boxreg_deltas[level] = box_deltas.flatten(2).permute(0, 2, 1)


            ctr_logits = self.pred_ctr(self.stem_box(feats_per_fpn_level[level]))
            centerness_logits[level] = ctr_logits.flatten(2).permute(0, 2, 1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # shape of features of each fpn level
        shape_per_fpn_level = {}
        ...
        matched_gt_deltas = [
            {
                ...
                ) if len(boxes) > 0 else torch.empty(0, 4)
        ...
        cls_idx = matched_gt_boxes[:, :, -1].to(torch.int64)
        
        imgs, locs = torch.where((cls_idx != -1))

        for img, loc in zip(imgs, locs):
            one_hot_gt[img, loc, cls_idx[img, loc]] = 1

        centerness = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4)).view_as(pred_ctr_logits)
        
        #calculates the losses
        loss_cls = sigmoid_focal_loss(pred_cls_logits, one_hot_gt)

        loss_box = 0.25 * F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction="none")
        
        # Mask for negative loss_box values
        loss_box[matched_gt_deltas < 0] = torch.tensor(0.0)

        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits, centerness, reduction="none")
        
        # Mask for negative centerness values
        loss_ctr[centerness < 0] = torch.tensor(0.0)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            ##################################################################
            # Feel free to delete this line: (but keep variable names same)
            level_pred_boxes = None
            level_pred_classes = None
            level_pred_scores = None  # Need tensors of shape: (N, 4) (N, ) (N, )
            ...
            # Step 2:
            # Replace "pass" statement with your code
            threshold_cross = (level_pred_scores > test_score_thresh).cpu()
            level_pred_scores = level_pred_scores[threshold_cross]
            level_pred_classes = level_pred_classes[threshold_cross]
            
            # Step 3:
            # Replace "pass" statement with your code
            level_locations = level_locations[threshold_cross]
            level_deltas = level_deltas[threshold_cross]
            level_pred_boxes = fcos_apply_deltas_to_locations(deltas=level_deltas,locations=level_locations,
                                                              stride=self.backbone.fpn_strides[level_name])
            level_pred_boxes = level_pred_boxes.cuda()
            
            
            # Step 4: Use images to get (height, width) for clipping.
            # Replace "pass" statement with your code
            h,w = images.shape[-2:]
            level_pred_boxes[:, [0, 2]] = torch.clamp(level_pred_boxes[:, [0, 2]], max=h)
            level_pred_boxes[:, [1, 3]] = torch.clamp(level_pred_boxes[:, [1, 3]], max=w)
